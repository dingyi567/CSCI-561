...
outputFile = open('sample_input_result.txt', 'w')
...
def func1(featureQueue, flag):
	...
	for j in diseasesName:
		...
		temp2 = 1 		   
		P3List = list()
		P3List.append(eval(linecache.getline("sample_input.txt", 4 * diseasesNO + 2 + flag)))
		for item1 in P3List:
			for item in item1:
		...
		P_result[j] = float("{0:.4f}".format(P_result[j]))
		flag += 1
	return P_result
...
def func2(flag):
	...
	for j in diseasesName:
		...
		flag3 = len(List)
		flag4 += flag3
		tempList1.append(float("{0:.4f}".format(min(resultList)))) 
		tempList1.append(float("{0:.4f}".format(max(resultList))))
		P_result[j] = tempList1
		flag += 1
	return P_result


def func4(j, temporary1):
	flag6 = 0
	for feature in wholeFeatureList:
		if (j + "/" + feature) in featureName:				
			if flag6 == temporary1:
				print("flag6:" + str(flag6))
...
def func3(flag):
	...
	for j in diseasesName:
		...
		for position, item in enumerate(resultList):
			...
			temp3 = (float(position) + 1) / 2
			print("temp3:" + str(temp3))
			temp = int(math.ceil(temp3))
			print("temp:" + str(temp))
			temporary1 = positionList[temp - 1]
			print("temporary1:" + str(temporary1))

			theFeatureName = func4(j,temporary1)
			print("theFeatureName:" + theFeatureName)

			print("position:" + str(position) + "featureName:" +theFeatureName + ":" +  str(compareResultDict[position]))
		...
		temp = int(math.ceil((float(IncreaseValueIndex) + 1) / 2))
		temporary1 = positionList[temp - 1]
		theFeatureName = func4(j,temporary1)
		...
		print("IncreaseValueIndex:" + theFeatureName)



		temp = int(math.ceil((float(DecreaseValueIndex) + 1) / 2))
		temporary2 = positionList[temp - 1]
		theFeatureName = func4(j,temporary2)
		resultList1.append(theFeatureName)
		...
		print("DecreaseValueIndex:" + theFeatureName)
